...
impl Contract {
    ...
    fn turn(
        ...
        let Some(new_fuel_left) = fuel_left.checked_sub((participant.payload_amount + 2 * self.weather as u8) / TOTAL_TURNS as u8) else {
        ...
        match turn_index {
            0 => {
                ...
                if participant.fuel_amount >= 80 - 2 * self.weather as u8 && random.chance(10) {
                    return Err(HaltReason::FuelOverload);
                }
            }
            1 => {
                if participant.payload_amount >= 80 - 2 * self.weather as u8 && random.chance(10) {
    ...
    async fn start_game(&mut self, participant: Participant) -> Result<Event, Error> {
        ...
        for turn_index in 0..TOTAL_TURNS {
            ...
            for (actor, participant) in &self.participants {
                let turn_entry = turn.entry(*actor).or_insert(Turn::Alive {
                    fuel_left: participant.fuel_amount,
                    payload_amount: participant.payload_amount,
                });

                let fuel_left = match turn_entry {
                    Turn::Alive { fuel_left, .. } => fuel_left,
                    Turn::Destroyed(_) => continue,
                };

                match self.turn(turn_index, &mut random, participant, *fuel_left) {
                    Ok(new_fuel_left) => *fuel_left = new_fuel_left,
                    Err(reason) => *turn_entry = Turn::Destroyed(reason),
                }
            }
...
async fn process_init() -> Result<(), Error> {
    let Initialize { master } = msg::load()?;
...
extern "C" fn state() {
    ...
    msg::reply(
        State {
            ...
            participants: participants.clone().into_iter().collect(),
            turns: turns
                .clone()
                .into_iter()
                .map(|turn| turn.into_iter().collect())
