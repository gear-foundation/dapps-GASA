...
export const RocketRace = (props: Props) => {
  const { xoffset = 10, bgColor = '#ADB2AF', id, sessionStatus, alive } = props;
  ...
  return (
    <div
      className="h-1/4 w-full border-2 border-b-gray-900"
      key={id}
      ...
      <span className="players-ready"> {`${handleAddressBySessionStatus(sessionStatus)}`}</span>
    </div>
  );
};
